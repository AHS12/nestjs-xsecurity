  ...
  if (config.rateLimit) {
    ...
    if (maxAttempts !== undefined) {
      if (!Number.isInteger(maxAttempts) || maxAttempts < 1) {
        throw new ConfigValidationError('rateLimit.maxAttempts must be a positive integer');
      }
    }

    if (decayMinutes !== undefined) {
      if (!Number.isInteger(decayMinutes) || decayMinutes < 1) {
        throw new ConfigValidationError('rateLimit.decayMinutes must be a positive integer');
      }
    }

    if (cleanupInterval !== undefined) {
      if (!Number.isInteger(cleanupInterval) || cleanupInterval < 1) {
        throw new ConfigValidationError('rateLimit.cleanupInterval must be a positive integer');
      }
    }
  }
  ...
  if (config.token) {
    ...
    if (expirySeconds !== undefined) {
      if (!Number.isInteger(expirySeconds) || expirySeconds < 1) {
        throw new ConfigValidationError('token.expirySeconds must be a positive integer');
      }
    }

    if (headerName !== undefined) {
      if (typeof headerName !== 'string' || !headerName.trim()) {
        throw new ConfigValidationError('token.headerName must be a non-empty string');
      }
  ...
  if (config.environment) {
    ...
    if (enabled !== undefined) {
      if (typeof enabled !== 'string' || !enabled.trim()) {
        throw new ConfigValidationError('environment.enabled must be a non-empty string');
      }
    }

    if (secret !== undefined) {
      if (typeof secret !== 'string' || !secret.trim()) {
        throw new ConfigValidationError('environment.secret must be a non-empty string');
      }
    }
  }
  ...
  if (config.errorMessages) {
    ...
    if (invalidToken !== undefined) {
      if (typeof invalidToken !== 'string' || !invalidToken.trim()) {
        throw new ConfigValidationError('errorMessages.invalidToken must be a non-empty string');
      }
    }
  }
